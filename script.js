document.addEventListener('DOMContentLoaded', function() {
    ...
    converterCategory.addEventListener('change', function() {
        ...
        switch(category) {
            ...
            case 'volume':
                ...
                break;
            case 'currency':
                fromOptions = '<option value="usd">USD</option><option value="eur">EUR</option><option value="gbp">GBP</option><option value="jpy">JPY</option>';
                toOptions = '<option value="eur">EUR</option><option value="usd">USD</option><option value="gbp">GBP</option><option value="jpy">JPY</option>';
                break;
        }
    ...
    function convertCurrency(value, from, to) {
        ...
        const rates = {
            usd: { eur: 0.93, gbp: 0.80, jpy: 149.31 },
            eur: { usd: 1.07, gbp: 0.86, jpy: 160.15 },
            gbp: { usd: 1.25, eur: 1.16, jpy: 186.41 },
            jpy: { usd: 0.0067, eur: 0.0062, gbp: 0.0054 }
        };
    ...
    function processVoiceCommand(command) {
        ...
        const commandMap = {
            ...
            'backspace': 'backspace', 'modulo': 'mod', 'ten power': 'tenx'
        };
    ...
    function handleMemory(operation) {
        ...
        showNotification(`Memory ${operation} performed`);
    }
    ...
    keys.forEach(key => {
        key.addEventListener('click', function(e) {
            ...
            if (['MC', 'MR', 'M+', 'M-', 'MS'].includes(btnVal)) {
                handleMemory(btnVal);
                return;
            }
            ...
            if (isScientificMode && ['sin', 'cos', 'tan', '√', 'x^y', 'π', 'x!', 'log', 'ln', 'e^x', 'tenx', 'mod'].includes(btnVal)) {
            ...
            if (btnVal === "C") {
                ...
                if (inputVal.length > 0) {
                    display.textContent = inputVal.slice(0, -1);
                    // Reset decimal flag if we removed a decimal point
                    if (inputVal.slice(-1) === ".") {
                        decimalAdded = false;
                    }
                }
